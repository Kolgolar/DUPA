...
func save_changes_to_timeline_file(path: String):
	...
	var data = {}
	# TODO:
	# Нужна отдельная кнопка для обновления списка спикеров. При этом,
	# должен кэшироваться старый массив спикеров и создаваться новый. После этого
	# находить РАЗНИЦУ в положениях одних и тех же персонажей внутри массивов
	# (мы берём именно разницу, потому что положение спикера в массиве не обязательно
	# будет равняться сохраняемому индексу, но если мы уверены, что будет наоборот, то пофиг).
