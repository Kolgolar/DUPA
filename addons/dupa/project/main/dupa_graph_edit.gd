...
func __duplicate_nodes(nodes_to_duplicate_ids: PackedInt32Array, forced_ids := PackedInt32Array()) -> PackedInt32Array:
	...
	for i in nodes_to_duplicate_ids.size():
		...
		if !forced_ids.is_empty():
			id = forced_ids[i]
		var new_node: DUPA_DefaultNode = _create_graph_node(node.scene_file_path, Vector2.ZERO, false, true, id)
...
func _on_mouse_popup_id_pressed(id:int):
	...
	ActionsMaster.register_property_action(op_name, self, "from_empty_to_node", "", _from_empty_to_node, false)
	ActionsMaster.register_property_action(op_name, self, "from_node_to_empty", "", _from_node_to_empty, false, UndoRedo.MERGE_ALL)
	ActionsMaster.register_property_action(op_name, self, "slot_to_connect", -1, _slot_to_connect, false, UndoRedo.MERGE_ALL)
...
func _create_graph_node(scene_path: String, pos := Vector2.ZERO, _is_start_node := false, remember_created := true, id := -1) -> GraphNode:
	...
	if _deleted_nodes.has(node.id):
		node.set_data(_deleted_nodes[node.id])
...
func set_timeline(timeline: Dictionary, config: Dictionary):
	...
	for graph_node in timeline:
		...
		node.set_data(timeline[graph_node])
		...
		for tag in timeline[graph_node]:
			...
			if from_port_num >= 0:
				...
				for go_to in timeline[graph_node][tag]: # get each in array
					connect_node(graph_names[graph_node], from_port_num, graph_names[timeline[graph_node][tag][go_to_count]], 0)
					go_to_count += 1
