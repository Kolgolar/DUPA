	...
	for id in blueprint_data:
		...
		match int(blueprint_node_data.type):
			...
			DUPA_Lib.NodeType.SETTER:
				dialog_node = DUPA_Lib.DN_Setter.new(blueprint_node_data)
			DUPA_Lib.NodeType.CALLER:
				dialog_node = DUPA_Lib.DN_Caller.new(blueprint_node_data)
...
func _go_to_node(dialog_node: DUPA_Lib.DN_Base) -> void:
	...
	match _curr_dialog_node.type:
		...
		nd.SETTER:
			var cdn := (_curr_dialog_node as DUPA_Lib.DN_Setter)
			...
			if action_set.get_connections().size() == 0:
				push_error("The signal 'action_set' has no connections!")
		nd.CALLER:
			var cdn := (_curr_dialog_node as DUPA_Lib.DN_Caller)
		...
		nd.CONDITION:
			...
			if action_condition.get_connections().size() == 0:
				push_error("The signal 'action_condition' has no connections!")
